package net.mg2013.vo
{
	...
	public class StyleSheetObject
	{
		...
		/**
		 *
		 * @param color - Only hexadecimal color values are supported. Named colors (such as blue) are not supported. Colors are written in the following format: #FF0000.
		 * @param display - Supported values are inline, block, and none.
		 * @param fontFamily - A comma-separated list of fonts to use, in descending order of desirability. Any font family name can be used. If you specify a generic font name, it is converted to an appropriate device font. The following font conversions are available: mono is converted to _typewriter, sans-serif is converted to _sans, and serif is converted to _serif.
		 * @param fontSize - Only the numeric part of the value is used. Units (px, pt) are not parsed; pixels and points are equivalent.
		 * @param fontStyle - Recognized values are normal and italic.
		 * @param fontWeight - Recognized values are normal and bold.
		 * @param kerning - Recognized values are true and false. Kerning is supported for embedded fonts only. Certain fonts, such as Courier New, do not support kerning. The kerning property is only supported in SWF files created in Windows, not in SWF files created on the Macintosh. However, these SWF files can be played in non-Windows versions of Flash Player and the kerning still applies.
		 ...
		 * @param letterSpacing - The amount of space that is uniformly distributed between characters. The value specifies the number of pixels that are added after each character. A negative value condenses the space between characters. Only the numeric part of the value is used. Units (px, pt) are not parsed; pixels and points are equivalent.
		 * @param marginLeft - Only the numeric part of the value is used. Units (px, pt) are not parsed; pixels and points are equivalent.
		 * @param marginRight - Only the numeric part of the value is used. Units (px, pt) are not parsed; pixels and points are equivalent.
		 * @param textAlign - Recognized values are left, center, right, and justify.
		 * @param textDecoration - Recognized values are none and underline.
		 * @param textIndent - Only the numeric part of the value is used. Units (px, pt) are not parsed; pixels and points are equivalent.
		 *
		 */
		public function StyleSheetObject(color:String = "#000000", display:String = "block", fontFamily:String = "Arial", fontSize:Number = 0, fontStyle:String = "normal", fontWeight:String = "normal", kerning:Boolean = false, leading:Number = 0, letterSpacing:Number = 0, marginLeft:Number = 0, marginRight:Number = 0, textAlign:String = "left", textDecoration:String = "none", textIndent:Number = 0)
		{
			super();
			__color = color;
			__display = display;
			...
			__kerning = kerning;
			__leading = leading;
			__letterSpacing = letterSpacing;
			__marginLeft = marginLeft;
			__marginRight = marginRight;
			__textAlign = textAlign;
			__textDecoration = textDecoration;
			__textIndent = textIndent;
		}
		...
		/* public function getAsTextFormat():TextFormat
		 {
			 var tf:TextFormat = new TextFormat(__fontFamily, __fontSize, __color, null, null, null, null, null, __textAlign, __marginLeft, __marginRight, __textIndent, __leading)
			 return tf;
		 }*/
